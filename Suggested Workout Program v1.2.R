...
available_equipment <- c("barbell","bench","dumbbell","rack","none",
                         "kettlebell","battle_rope","bike","rowing_machine","ab_wheel",
                         "dip_bars","ez_bar","tricep_bar","cable",
                         "bands","vest","belt","pull-up bar","trap_bar")
                #From:  #battle rope, jump rope, heavy rope, gloves, bike, 
                        #rowing_machine, ab_wheel, bar, dip_bars, machine, 
...
# Possible workouts
all_types <- c("Shoulders","Chest","Back","Bicep","Legs","Arms","Push","Pull","Full_Body")
all_intensity <- c("Easy","Medium","Hard","Heavy","Light","Quick")
...
Random <- function(X){
  sample(X,1)}
Type <- Random(all_types)
Intensity <- Random(all_intensity)
Random_Workout <- c(Type,Intensity)
...
# Read ALL log filenames, sort by "most recently modified" (most recent first)
log_files <- list.files(output_dir, pattern = "\\.txt$", full.names = TRUE)
log_files <- log_files[order(file.info(log_files)$mtime, decreasing = TRUE)]

# Keep only the 2 most recent logs for decision-making
last_two_files <- head(log_files, 2)

# Parse Type / Intensity / Date from filename
# Expected filename pattern:
#   TYPE[_TYPE]_INTENSITY_YYYY-MM-DD_SUGGESTED.txt

parse_log <- function(path) {
  name  <- tools::file_path_sans_ext(basename(path))
  parts <- strsplit(name, "_", fixed = TRUE)[[1]]
  
  # Identify the date token explicitly (YYYY-MM-DD)
  date_idx <- grep("^\\d{4}-\\d{2}-\\d{2}$", parts)
  
  # If no valid date is found, return NAs safely
  if (length(date_idx) != 1 || date_idx < 2) {
    return(list(type = NA_character_, intensity = NA_character_, date = as.Date(NA)))
  }
  
  # Intensity is the token immediately before the date
  intensity <- parts[date_idx - 1]
  
  # Type is everything before intensity (may include underscores)
  type <- paste(parts[seq_len(date_idx - 2)], collapse = "_")
  
  list(
    type      = type,
    intensity = intensity,
    date      = as.Date(parts[date_idx], format = "%Y-%m-%d")
  )
}


# Parse ONLY the 2 most recent logs
hist <- lapply(last_two_files, parse_log)

# Pull vectors
hist_type <- vapply(hist, `[[`, character(1), "type")
hist_int  <- vapply(hist, `[[`, character(1), "intensity")
hist_date <- as.Date(vapply(hist, function(x) as.character(x$date), character(1)))

# Map specific types into 4 general categories
# Categories:
#   - Push      = Chest, Shoulders, Push
#   - Pull      = Back, Bicep, Arms, Pull
#   - Legs      = Legs
#   - Full Body = Full_Body
type_to_category <- function(t) {
  if (is.na(t)) return(NA_character_)
  
  # If a logged type contains 2 types (e.g., "Legs_Pull"),
  # keep BOTH tokens and map them to categories below.
  type_tokens <- strsplit(t, "_", fixed = TRUE)[[1]]
  
  # If any token is Full_Body, treat as Full Body
  if (any(type_tokens == "Full_Body")) return("Full Body")
  
  # If any token is Legs, treat as Legs
  if (any(type_tokens == "Legs")) return("Legs")
  
  # If any token is in Push set, treat as Push
  if (any(type_tokens %in% c("Chest","Shoulders","Push"))) return("Push")
  
  # If any token is in Pull set, treat as Pull
  if (any(type_tokens %in% c("Back","Bicep","Arms","Pull"))) return("Pull")
  
  NA_character_
}

# Types available within each category (used when we sample a concrete Type)
cat_to_types <- list(
  "Push"      = c("Chest","Shoulders","Push"),
  "Pull"      = c("Back","Bicep","Arms","Pull"),
  "Legs"      = c("Legs"),
  "Full Body" = c("Full_Body")
)

###
### Choose Type (category-based, using only the 2 most recent logs)
###

# Full_Body is ONLY allowed if the most recent logged date is > 10 days ago
last_date <- if (length(hist_date) && !is.na(hist_date[1])) hist_date[1] else NA
days_since_last <- if (!is.na(last_date)) as.integer(Sys.Date() - last_date) else 0L

if (!is.na(last_date) && days_since_last > 10L) {
  # Long break: suggest Full Body
  Type <- "Full_Body"
} else {
  # Otherwise, avoid repeating the last 2 categories (Push/Pull/Legs only)
  last_two_cats <- unique(na.omit(vapply(hist_type, type_to_category, character(1))))
  
  eligible_cats <- setdiff(c("Push","Pull","Legs"), last_two_cats)
  
  # Fallback if last two sessions already hit all 3 categories
  if (!length(eligible_cats)) eligible_cats <- c("Push","Pull","Legs")
  
  chosen_cat <- sample(eligible_cats, 1)
  Type <- sample(cat_to_types[[chosen_cat]], 1)
}

###
### Choose Intensity (based only on last 2 intensities)
###

# Never output Quick
allowed_intensity <- setdiff(all_intensity, "Quick")
last_intensity <- if (length(hist_int)) hist_int[1] else NA_character_

# If last intensity was Medium, keep Medium
if (!is.na(last_intensity) && last_intensity == "Medium") {
  Intensity <- "Medium"
} else {
  # Otherwise, avoid repeating intensities used in the last 2 sessions (if possible)
  recent_int <- unique(na.omit(hist_int[hist_int %in% allowed_intensity]))
  eligible_int <- setdiff(allowed_intensity, recent_int)
  
  # Fallback if exclusions remove everything
  if (!length(eligible_int)) eligible_int <- allowed_intensity
  
  Intensity <- sample(eligible_int, 1)
}

# Output
suggested_workout <- c(Type = Type, Intensity = Intensity)
suggested_workout

#############################
### Use Suggested Workout ###
#############################

# Use the suggested workout output (from your suggested_workout code)
target_type    <- suggested_workout["Type"]
intensity_name <- suggested_workout["Intensity"]
